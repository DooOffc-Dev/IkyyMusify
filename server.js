...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist}` : qTitle) : `Dengarkan Musik - IkyyMusify`;
            const playDesc = `Dengarkan ${qTitle || 'lagu favoritmu'} di IkyyMusify Web Music Player`;
    ...
    if (req.path.startsWith('/artist/')) {
        ...
        if (artistId) {
            ...
            const pageTitle = qName ? `${qName} (Artist) - IkyyMusify` : `Artist - IkyyMusify`;
            const pageDesc = qName ? `Dengarkan lagu & album terbaik dari ${qName} di IkyyMusify` : `Dengarkan lagu & album dari artist favoritmu di IkyyMusify`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/cShqETwvDGiJ.jpg`;
    ...
    if (req.path.startsWith('/album/')) {
        ...
        if (albumId) {
            ...
            const pageTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist} (Album) - IkyyMusify` : `${qTitle} (Album) - IkyyMusify`) : `Album - IkyyMusify`;
            const pageDesc = qTitle ? `Dengarkan album ${qTitle} di IkyyMusify` : `Dengarkan album favoritmu di IkyyMusify`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/cShqETwvDGiJ.jpg`;
