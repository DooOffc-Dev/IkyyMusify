// ============================================================
// IkyyMusify - DYNAMIC OG META INJECTION (Edge Function)
// ============================================================
...
// real preview from. Those links keep the default IkyyMusify preview.

const FALLBACK_IMAGE = 'https://www.gobox.my.id/file/cShqETwvDGiJ.jpg';
...
async function getTrackMeta(origin, videoId) {
    ...
    var title = 'Lagu', artist = 'IkyyMusify', cover = hdCover(null, videoId);
    ...
    return {
        title: title,
        description: 'Dengarkan ' + title + ' - ' + artist + ' di IkyyMusify',
        image: cover
    };
}
...
async function getArtistMeta(origin, artistId) {
    ...
    return {
        title: name + ' (Artist)',
        description: 'Dengarkan lagu & album terbaik dari ' + name + ' di IkyyMusify',
        image: image
    };
}
...
async function getAlbumMeta(origin, albumId) {
    ...
    return {
        ...
        description: 'Dengarkan album ' + fullTitle + ' di IkyyMusify',
        image: image
    };
}
...
function injectMeta(html, meta, pageUrl) {
    html = html.replace(/<title>[^<]*<\/title>/, '<title>' + escapeAttr(meta.title) + ' - IkyyMusify</title>');
    html = replaceMetaContent(html, /<meta property="og:title"[^>]*>/, meta.title + ' | IkyyMusify');
